""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"bu kodu iki farklı şekilde yazdım fakat hatamı bulamadım gene de yükledim
"sorunlar şunlar:
"1)takımları karma şekilde dağıtamadım
"2) ya da tabloda sadece bir takım gözüktü
...
LOOP AT gt_team INTO gs_team.
  IF gs_team-torba_id = 1 AND lines( gt_group1 ) < 4.
    lv_exists = abap_false.
    LOOP AT gt_group1 INTO DATA(ls_group1).
      IF ls_group1-country = gs_team-country.
        lv_exists = abap_true.
        EXIT.
      ENDIF.
    ENDLOOP.
  ...
  ELSEIF gs_team-torba_id = 2 AND lines( gt_group2 ) < 4.
    lv_exists = abap_false.
    LOOP AT gt_group2 INTO DATA(ls_group2).
      IF ls_group2-country = gs_team-country.
        lv_exists = abap_true.
        EXIT.
      ENDIF.
    ENDLOOP.
  ...
  ELSEIF gs_team-torba_id = 3 AND lines( gt_group3 ) < 4.
    lv_exists = abap_false.
    LOOP AT gt_group3 INTO DATA(ls_group3).
      IF ls_group3-country = gs_team-country.
        lv_exists = abap_true.
        EXIT.
      ENDIF.
    ENDLOOP.
  ...
  ELSEIF gs_team-torba_id = 4 AND lines( gt_group4 ) < 4.
    lv_exists = abap_false.
    LOOP AT gt_group4 INTO DATA(ls_group4).
      IF ls_group4-country = gs_team-country.
        lv_exists = abap_true.
        EXIT.
      ENDIF.
    ENDLOOP.
